...
\subsection{RDMA performance constraints}
...
implementing locks or opportunistic concurrency.  Atomic operations can address
at most 64 bits at a time and bottleneck at lower rates than reads and
...
millions of RDMA operations per second (limited only by link rate, not shown),
but CAS operations to remote server memory top out around 50 MOPS.
%Hence, we employ atomic operations judiciously in RCuckoo.

%the scalability of
%these operations on CX5 NICs for 64 bit operations.
%%

Our testbed NICs, however---like those from several
...
atomic operations.  Figure~\ref{fig:rdma-benchmarks}(b) shows the
...
faster on NIC memory.  RCuckoo performs CAS operations only on datstructures in NIC memory.

Finally, we note that while atomic operations are limited to 64 bits, read and write message
...
Figure~\ref{fig:rdma-benchmarks}(c) shows that on our testbed
...
single larger one when appropriate.  Selecting the optimal threshold is a trade-off
between latency reduction and I/O amplification.
...
We use cuckoo rather than hopscotch hashing in RCuckoo due to locking
concerns.
%While both alog
