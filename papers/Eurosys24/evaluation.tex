...
table and prepopulate it with 90~M entries that each consist of a 32-bit key and 32-bit value (we
...
pattern.  (Sherman performance is improves---but does not surpass RCuckoo---for uniform workloads,
...
\begin{figure*}[t]
    \centering
    % \begin{subfigure}{0.24\linewidth} % orignal width for 4 figures
...
RCuckoo's performance on workloads that exclusively use inserts (rather than updates), for example
in this instance A is 50\% insert and 50\% read. W is insert only. Inserts become more expensive as the table fills, so we prepopulate the table with a
...
\textbf{Latency and overhead.} We plot operation latency, size, and RDMA message counts as a
function of fill factor in Figure~\ref{fig:ycsb_fill}(b--d). We report latency values of insert and
read operations for the comparison systems. These values were not collected under load and represent
best-case performance on our testbed.  Read latency is nearly identical for all systems save FUSEE,
...
(Figure~\ref{fig:ycsb_fill}(c)) and messages (Figure~\ref{fig:ycsb_fill}(d)) per operation.  Reads,
on the other hand, have roughly the same cost and performance regardless of fill factor.
...
\textbf{Entry size.} \label{sec:entry_size} 
%%
...
and YCSB-C (read-only) workloads. Insert is a bandwidth-limited operation;
%quickly saturates the network bandwidth. At
at 16-B entries link capacity restricts throughput to 8 MOPS. The performance of Reads/Update/Delete
% (and
% updates/deletes, not shown), 
are largely unaffected by entry size.  Hence,
...
Figure~\ref{fig:microbenchmarks}(e) shows extent performance on value sizes from 8 to 1024 bytes.
The dashed line marks the performance of inlined 8byte values (16 byte entries) as a base line.
This benchmark was taken on 6 client machines with 120 cores with zipf 0.99 distribution and GC off.
Inlined entries have two sources of performance gain, first they avoid the overhead of read and
writing to extents, but most importantly they avoid the overhead of additional rounds trips on cache
misses. YCSB-B inline ops have a 21\% performance improvement and YCSB-A has a 37\% performance
improvement (YCSB-C has no cache misses).
