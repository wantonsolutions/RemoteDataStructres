...
shown).  As locks begin to cover additional rows, however, the second search becomes much more
useful.
...
\textbf{Entry size.} \label{sec:entry_size} Inlined key/value entries enable single-round-trip
reads, so the larger the entries, the fewer values that require a second round trip.  However, for a
...
at 16-B entries link capacity restricts throughput to 8 MOPS. The performance of reads (and
updates/deletes, not shown), are largely unaffected by entry size.  Hence,
...
read-heavy services may wish to employ larger entry sizes.
%
