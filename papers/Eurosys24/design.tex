...
Table entries contain either inlined key/value pairs a key and 48-bit
pointer to an extent entry for larger values.  The least-significant bit of an entry signifies its type; values are inlined by default.
%which are used by
%default (and signified by a 0 LSB) or 
Entry and key sizes are configuration parameters, but must be fixed at
table creation.  We use 8-byte entries in our experiments, with 4-byte
keys and 4-byte values.
...
Assuming successful lock acquisition and valid reads, the operation can proceed if the key is
