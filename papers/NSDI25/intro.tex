...
\begin{figure*}[t]
    ...
    \textbf{(c)} Extent vs Inline performance on YCSB workloads for small values (C=100\% reads, B=95\%read 5\%write A=50\%read 50\% write).
    \textbf{(d)} compare-and-swap performance on device and main memory.
    }
