...
key $K$ (which requres the cuckooing of key $C$).  The index table (right) is a single region of
...
Table entry and key sizes are configuration parameters, but must be fixed at
table creation.  Regardless of their size, entries contain either inlined key/value pairs (bottom left) or a key and
...
RDMA-registered cache of (relevant portions\footnote{A small cache suffices; we use 64~KB in our experiments. Caching the entire index yields negligible additional benefit.} of) the index table.
...
succeed upon retry.  It also reduces the region
