...
by around 1.5x (~\todo{cite design guidelins and some
connectx 7 spreadsheets}). Comparatively bandwidth is
...
To do so we make our two hash functions \textbf{dependent}
...
Instead of a static bound we use a dynamic exponential bound
with a third hash function. The bound set by the third hash
is determine by counting the suffix zeros of the resultant
hash and rasing it by an exponential factor. In the common
case the bound is small, but at an exponentially decrease
rate some pairs of values are spaced exponentially far
apart. This design enables some pairs to act
as~\textit{waypoints} to other regions of the table when
constructing cuckoo paths. This method, paired with bucket
associativity enables high fill rates while keeping the
region of the table any given key can inhabit small.
...
CAS, and curved dashed lines are writes. \todo{talk to this figure}}
...
reduce the number of messages at the cost of more bandwidth.}
