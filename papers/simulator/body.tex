...
of the table, and retry the insert.\sg{Further - Issuing CAS as a
batch leads to complex path failure cases such a single
element in the path failing while others further down the
path succeed. Assesing and fixing such insertion failures
without locks is very hard.}
...
hash table throughput.~\todo{Figure ref global shows how
concurrent performance scales with lock granularity.}
...
aquire locks. Figure~\todo{insert a lock range figure which
shows the number of random lock locations and the number of
locally bounded locations}.
