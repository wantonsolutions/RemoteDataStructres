...
\textbf{Speculative local search:} 

We use a heuristic search to find cuckoo paths. Starting with an empty cache, we
assume the row a key hashes to has an open slot (path length 1). We then lock
the and read the rows the lock protects (cache syncronization). We retain the
cache temporarily. If the insertion can't be completed with one lock we try
again this time speculatively searching on cached rows.
%%
