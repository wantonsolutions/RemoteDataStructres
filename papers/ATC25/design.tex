...
in practice) and terminates with an 8-bit version number (not shown) and 64-bit
...
speculative cuckoo path.  As with prior work~\cite{cuckoo-improvements}, we use breadth-first
search (BFS) to identify short paths in an attempt to minimize
bandwidth and locking overhead.  If the
...
our evaluation we set $f=2.3$.  As shown in the figure, for index
...
concern if the failure occurs while the client was holding one or more locks, i.e., in the middle of a mutating operation
