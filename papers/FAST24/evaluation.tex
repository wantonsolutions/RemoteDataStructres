...
(Figure~\ref{fig:ycsb_fill_latency}(b-d)). In all cases
inserts are more expensive as the table fills, but have
little impact on read performance. As the table fills,
cuckoo paths grow in length causing an increase both in
bandwidth consumption per operation but also latency as
additional round trips are required to find valid cuckoo
paths. In all cases we use a lock size of 16 rows, and BFS
search. Both of reduce the cost of inserts significantly as
