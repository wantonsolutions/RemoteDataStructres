...
RCuckoo is designed to facilitate lock-less, single round-trip reads for small values as
they are the dominant operation for key/value stores in data
centers~\cite{facebook-memcached,rocks-db-workload}. To read the value associated with a
...
Due RCuckoo's locality enhancement, it is usually possible to attempt
...
the same batch of operations.  (In the rare case that the locks must
...
rows.  Unlike with updates and deletes, it is unlikely that all of the
locks required for a complete cuckoo path can be acquired with a
single MCAS operation, so locks are acquired in groups (see
Section~\ref{sec:locking}). Immediately after, but in the same batch
of RDMA operations as each attempt to acquire (a subset of) the locks,
clients synchronize their local cache by issuing reads for (at least)
all of the rows covered by that set of locks.  (In general, locks cover
multiple rows, so this will be a superset of the rows necessary for
the identified path.)  Note that if lock acquisition is successful,
the values returned by the read of the corresponding rows---and, thus,
the client's local cache of those rows---will remain synchronized
...
being evicted its secondary location.  Hence, the client has
...
% hash table.
%In principle, the cache can be maintained not just across retries but
%also across insert operations for different keys, but
...
detected (Section~\ref{sec:table-repair}).
...
%    \end{subfigure}
 ...
 %   \textbf{(a)} Dependent hashing for factor $f$.
    ...
    \textbf{(b)} Achieved maximum fill percentage for different locality settings. 90\% fill indicated by dashed red line.
        \textbf{(c)} CDF of cuckoo spans for dependent and independent hashing. A cuckoo span is the distance between the smallest and largest index in a cuckoo path.~\todo{buckets to rows}
    }
...
In RCuckoo, the first location for a key chosen uniformly at random,
while the second is selected as an offset from the first chosen
uniformly at random from a probabilistically bounded range, where the
...
increased distances from their first.  The latter aspect is crucial, as any
...
our experiments we set $f=2.1$.  As shown in the figures, for index
...
paths that will frequently fit within the rows returned from a covering
read.
%
...
\begin{figure}[t]
  \vskip -1em
    ...
    when filling to 95\%. 512 buckets per lock is a single global lock.}
    \label{fig:cuckoo-problems}

\end{figure}

Increased locality also decreases the number of round trips required
...
of sets as possible (where each set is an attept to acquire one or
...
necessary lock(s).  (In the extreme, there would just be one, global
...
experiment where \todo{XXX} clients are concurrently filling a 512-row
...
that can be access with a single MCAS operation, on the
...
operations to device memory avoid a PCIe round trip.\footnote{It also
allows RCuckoo clients to employ \texttt{SEND\_FENCE} on covering
reads batched alongside lock acquisitions without incuring additional
performance overhead.}  Unfortunately, NIC memory is limited (to
...
Once identified, real-world deployments likely have many ways to
