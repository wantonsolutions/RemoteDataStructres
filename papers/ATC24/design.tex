...
fewer rows while nearly 99\% span 256 or fewer.  This result implies
that paths will freqently fit within the rows returned from a covering
read.
%
...
In addition, to imporove locking performance, RCuckoo locates the lock
...
To avoid deadlock RCuckoo aquires locks in increasing physical order.  Given
...
Due to this one-MCAS-per-roundtrip lock acquition procedure, lock granularity has a pivotal role on performance.
...
row shows the 99-th percentile round trips needed for inserts which
aquire a single lock at a time. In the top left fine-grained locking with one
lock per message results in 55 round trips. The far right column shows
that a single global lock results in high contention. Dense tables with
few rows per lock result in the fewest round trips.  In our
experiments we use 16 rows per lock as it has a negligible effect on
contention and requires a single lock to be acquired for vast majority
of insertions.
...
locks), RCuckoo clients are given a bounded time to aquire all their
locks. If a client takes longer than this time to aquire all of its
