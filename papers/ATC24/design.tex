...
\begin{figure}[t]
  \vskip -1em
    \centering
        \includegraphics[width=0.99\linewidth]{fig/buckets_per_lock_vs_locks_per_message.pdf}
    \vspace{-1em}
    \caption{Round trips (99th percentile) on a table
    with 512 rows required per insert while filling a table
    to 95\%. 512 buckets per lock is a single global lock.}
...
Figure~\ref{fig:locality-hashing} shows that when filling the table
...
that paths will frequently fit within the rows returned from a covering
read.
%
...
To avoid deadlock RCuckoo acquires locks in increasing physical order.  Given
...
\todo{Not sure this comports with Figure 5(c).}
