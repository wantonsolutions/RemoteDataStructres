...
across all settings, while the other systems each shine on different
mixes.
\todo{What is the row/read size?  How close are we to line rate?}

\textbf{YCSB:} Figure~\ref{fig:ycsb_throughput} shows throughput in
...
YCSB-B is 95\% read and 5\% update, and YCSB-C is 100\% read.  In each
experiment a variable number of clients access 90~M entries according
to a Zipf(0.99) distribution, where each entry consits of a 32-bit key
alongside an inlined 32-bit value.  All entries are stored in a table
with a total capacity of 100~M entires.
