...
while insert performance varies as a function of table fill factor.  \todo{What is
  the row/read size?  How close are we to line rate?}
...
\begin{figure*}[t]
    ...
    \caption{RCuckoo parameter sweeps:
      \textbf{(a)} YCSB-A throughput vs client failure rate.,
          \textbf{(b)} Failure rate of search algorithms vs lock size under high contention,
    \textbf{(c)} Failure rate of search algorithms vs lock size under high contention, and
    \textbf{(D)} throughput vs key/value entry size for YCSB-A (insert) and YCSB-C.
    }
...
Figure~\ref{fig:microbenchmarks}(a) illustrates RCuckoo's
...
Inlined key/value entries enable RCuckoo's single-round-trip
...
size on operation throughput under YCSB-A. Insert is a
...
shown), on the other hand, are largely unaffected by entry
size.  Hence,
...
%reads see a large
installations serving read-heavy workloads may wish to
