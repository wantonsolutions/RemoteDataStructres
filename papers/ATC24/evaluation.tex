...
Client failures can strand locks and reduce overall system
performance. We measure RCuckoo's performance in the face of failures
by injecting stranded locks.  We emulate client failures by performing
a partial insert operation that issues a random prefix of the RDMA
operations rquired for the insert. This process leaves the table in
one of the recovery states and strands at least one lock. In our
experiment the table is filled to 80\%, non-failing clients employ a
YCSB-A workload, and the fault-injection client is given a fixed rate
at which to inject faults.

Figure~\ref{fig:failure_throughput} shows the degradation in aggregate
throughput as a function of failure rate. The more rows a lock
protects the larger the failure's impact and time to triage and repair
the failure. The performance impact of a few number of failures per
second is low which is ideal considering that failures in RDMA
clusters are relatively rare.  As a point of refrence, we observe that
RCuckoo can still operate despite thousands of failures per second
while a server can only establish approximately 1.4~K RDMA connections
per second using state of the art techniques~\cite{xrdma}.
