...
%%
These writes have the advantage that updates are atomically
visible, no critical sections exist, and client failures do
not leave the table in an inconsistent state. However,
opportunistic concurrency comes at a cost. Contention leads
to frequently failed operations and high tail
latencies~\cite{clover}, CAS operations don't scale
well~\cite{design-guidelines}(Figure~\ref{fig:rdma-benchmarks}(b))
and 64 bit width CAS forces tables to entries to be pointers
to keys and values rather than values
themselves~\cite{race}. Using pointers in a key-value index
rather than directly inlining key-value pairs requires two round trips for
