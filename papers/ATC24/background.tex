...
potential location(s) of an entry in the table, regardless of
...
\textbf{Cuckoo hashing} uses two or more independent hash functions to
assign a set of potential table locations for a key, where locations
correspond to an associative row of entries.  RCuckoo uses two
locations, a primary and a secondary.  When a key is inserted, if its
primary row is full an existing key is evicted to its alternative
row. If the eviction causes another collision the process iterates
until an open location is found. The path of evictions is known as
a~\textit{cuckoo path}.  While insertions can be involved, reads can
always be executed in a single round trip by reading the rows
corresponding to both of a key's locations
...
This process is typically facilitated by maintaining a per-entry
bitmask of nearby collisions.  Like cuckoo hashing, reads can be
conducted in a single round trip time by reading the entire
...
the entry being moved to its alternate location---rather than two.
...
only two locks, while hopscotch entries inhabit a range of locations,
so a conservative locking strategy must lock the entire range.  That
said, RCuckoo takes inspiration from hopscotch hashing's neighborhoods
and employs dependent hashing to increase the spatial locality of
keys' potential locations.
